...
#
# datasets = ['20ng', 'R8', 'R52', 'ohsumed', 'mr','微博']
# # build corpus
# dataset = sys.argv[1]
#
...
#     words = jieba.lcut(doc_words)  # 进行分词
#     print(words)
#     for word in words:
#         word_set.add(word)
...
#
# label_list_str = '\n'.join(label_list)#.join是使得列表元素之间加一个字符，这里面加的就是回车符
...
#
# #实际训练数据的句子标签和训练种类排列例如（362，train，0）
...
#创建一个word——frequency这是一个词和对应数量的字典
word_doc_freq = {}
for word, doc_list in word_doc_list.items():
    word_doc_freq[word] = len(doc_list)
